...
class Section(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.order:
            self.order = (
                max([x.order for x in self.portfolio.sections.all()], default=0.0) + 0.1
            )
...
class Task(models.Model):
    ...
    duration = models.PositiveSmallIntegerField(
        help_text="Typical duration of this task"
    )
    completion_days = models.PositiveSmallIntegerField(
        help_text="Expected days to complete a WorkItem with this task",
        default=7
    )
    created = models.DateTimeField(auto_now_add=True, editable=False)
    created_by = models.ForeignKey(
...
class WorkItem(models.Model):
    ...
    assigned_to = models.ForeignKey(
        User, models.DO_NOTHING, null=True, blank=True, related_name="assigned_users"
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.order:
            self.order = (
                max([x.order for x in self.section.workitems.all()], default=0.0) + 0.1
            )
